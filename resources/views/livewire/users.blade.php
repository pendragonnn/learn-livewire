            ...
            <div class="mt-4">
                <form wire:submit.prevent="createNewUser" action="#" method="POST" class="space-y-6">
                    ...
                    <div wire:loading wire:target="avatar"
                        class="flex items-center justify-center h-20 w-20 border border-gray-200 rounded-lg bg-gray-50">
                        <div
                            class="px-2 py-px text-[10px] font-medium leading-none text-center text-blue-800 bg-blue-200 rounded-full animate-pulse">
                            loading...</div>
                    </div>
                    ...
                    @if ($avatar)
                        ...
                        <img class="rounded w-20 h-20 block object-cover" src="{{ $avatar->temporaryUrl() }}" alt="">
                    @endif
                    ...
                    <div>
                        <button wire:click.prevent="createNewUser"
                            ...
                            <svg wire:loading wire:target="createNewUser" aria-hidden="true"
                                class="w-4 h-4 self-center me-2 text-gray-200 animate-spin fill-blue-600"
            ...
            @foreach ($users as $user)
                <li class="flex justify-between gap-x-6 py-5">
                    <div class="flex min-w-0 gap-x-4">
                        <img src="{{ $user->avatar ? asset('storage/' . $user->avatar) : asset('img/default-avatar.jpg') }}"
                            alt="User Avatar" class="size-12 flex-none rounded-full bg-gray-50" />
                        <div class="min-w-0 flex-auto">
                            <p class="text-sm/6 font-semibold text-gray-900">{{ $user->name }}</p>
                            <p class="mt-1 truncate text-xs/5 text-gray-500">{{ $user->email }}</p>
                        </div>
                    </div>
                    <div class="hidden self-center shrink-0 sm:flex sm:flex-col sm:items-end">
